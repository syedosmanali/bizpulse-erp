...
def get_all_sales():
    """Get all sales entries with proper date filtering - ISO 8601 + Asia/Kolkata timezone"""
    from datetime import datetime, timedelta
    ...
    # Get current date in local time (assuming server is in IST)
    now = datetime.now()
    ...
    # Build query with proper date range (00:00:00 to 23:59:59)
    query = '''
        SELECT 
            s.id,
            s.bill_id,
