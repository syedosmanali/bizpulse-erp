    ...
    conn = get_db_connection()
        ...
        def get_products_report():
            ...
            try:
                ...
                for product in products_data:
                    ...
                    # Performance scoring
                    revenue_score = min(product['revenue'] / 1000 * 20, 40)
                    units_score = min(product['units_sold'] * 3, 30)
                    stock_score = 20 if product['current_stock'] > 0 else 0
                    transaction_score = min(product['transactions'] * 2, 10)
        ...
        def get_customers_report():
            ...
            try:
                ...
                return jsonify({
                    'customers': [dict(row) for row in customers_data],
                    'dateRange': {'from': from_date, 'to': to_date}
                })
