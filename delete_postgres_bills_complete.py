...
try:
    ...
    if bills:
        ...
        # Delete from all tables that reference bills
        print("\nDeleting related records from all tables...")
        
        # Delete from sales table
        cursor.execute("DELETE FROM sales WHERE bill_id IN %s", (bill_ids_tuple,))
        ...
        print(f"\nDeleted {deleted_bills} bills from today")
        print(f"Total records deleted: {deleted_sales + deleted_payments + deleted_items + deleted_bills}")
    else:
