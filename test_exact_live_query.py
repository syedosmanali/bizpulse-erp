"""
Test the EXACT query that the live API is running
"""
import os
...
print("🔍 Testing EXACT live query...")

try:
    conn = psycopg2.connect(DATABASE_URL, sslmode='require')
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    
    # Test with DEFAULT filter (today)
    print("\n📊 Testing with date_filter='today' (default):")
    ...
    query = """
        ...
        WHERE b.business_owner_id = %s AND CAST(b.created_at AS DATE) = %s
        ORDER BY b.created_at DESC
    """
    
    cursor.execute(query, ('admin-bizpulse', today))
    sales = cursor.fetchall()
    
    print(f"✅ Query successful! Found {len(sales)} sales for today")
    for sale in sales[:3]:
